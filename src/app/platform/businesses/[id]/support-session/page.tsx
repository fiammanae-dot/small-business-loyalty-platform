...
import { startSupportSessionAction } from "@/app/platform/businesses/support-actions";

export default async function StartSupportSessionPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <DashboardShell user={user} eyebrow="System Administrator" title="Open support session">
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      ...
      <p className="mt-1 break-words font-semibold text-[#111827]">{value}</p>
    </div>
  );
}