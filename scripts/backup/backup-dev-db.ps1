...
Write-Host "Creating development database backup..."
Write-Host "Database: loyalty_platform"
Write-Host "Output: $BackupFile"

& $PgDump -U postgres -h $DbHost -p $DbPort -d loyalty_platform -Fc -f $BackupFile

if ($LASTEXITCODE -ne 0) {
  Write-Error "Backup failed for loyalty_platform."
}
