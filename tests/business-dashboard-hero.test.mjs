...
test("business dashboard hero uses approved display helpers for title and type", () => {
  ...
  assert.match(dashboard, /\{businessDisplayName\}/);
  assert.match(dashboard, /\{businessTypeDisplayName\}/);
  assert.doesNotMatch(dashboard, /\{business\.name\}/);
  assert.doesNotMatch(dashboard, /businessDisplayName=\{[^}]*(?:updatedAt|createdAt|id)[^}]*\}/);
