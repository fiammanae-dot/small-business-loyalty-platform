...
test("operations center support module exposes session management and reports", function () {
  ...
  const requestDetails = read("src/app/platform/operations-center/requests/[id]/page.tsx");
  ...
  assert.match(nav, /Operations Center/);
  assert.match(nav, /\/platform\/operations-center/);
  ...
  assert.match(operations, /function SessionPass/);
  assert.match(operations, /function RequestPass/);
  ...
  assert.match(operations, /Top reason/);
  assert.match(operations, /Total sessions/);
  assert.match(operations, /Most supported businesses/);
  assert.match(operations, /Start Support Session/);
  assert.match(operations, /\/platform\/operations-center\/requests\/\$\{request\.id\}/);
  assert.match(operations, /\/platform\/operations-center\/support\/\$\{session\.id\}/);
  assert.match(operations, /Open support request details/);
  assert.match(operations, /Open support session details/);
  assert.doesNotMatch(operations, /TerminateSupportSessionButton/);
  assert.doesNotMatch(operations, /joinSupportSessionAction/);
  assert.doesNotMatch(operations, /View Report/);
  ...
  assert.match(requestDetails, /Support Request Details/);
  assert.match(requestDetails, /Request Information/);
  assert.match(requestDetails, /Support Session/);
  assert.match(requestDetails, /Start Session/);
  assert.match(requestDetails, /joinSupportSessionAction/);
  assert.match(requestDetails, /TerminateSupportSessionButton/);
  assert.match(report, /Support Session Report/);
  assert.match(report, /joinSupportSessionAction/);
  assert.match(report, /TerminateSupportSessionButton/);
...
test("business owners can view support history without internal audit details", function () {
  ...
  assert.match(page, /formatDateTime\(session\.startedAt\)/);
  assert.match(page, /session\.supportSummary/);
  assert.match(page, /No summary recorded yet\./);
  assert.match(page, /formatSupportDuration\(session\.startedAt, session\.endedAt \?\? session\.expiresAt\)/);
  assert.match(page, /https:\/\/wa\.me\/971505009707/);
  assert.match(page, /Contact Loyalty Card UAE support on WhatsApp/);
  assert.match(page, /Hello Loyalty Card UAE Support, I need help with my account\./);
  ...
  assert.match(actions, /Loyalty Card UAE Support accessed your workspace/);
});
