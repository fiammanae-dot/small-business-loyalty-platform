...
test("platform users master-detail experience keeps lifecycle controls on the detail page",function(){const page=read("src/app/platform/users/page.tsx");const detail=read("src/app/platform/users/[id]/page.tsx");assert.doesNotMatch(page,/Read-only/);assert.doesNotMatch(page,/PlatformUserPasswordResetAction/);assert.doesNotMatch(page,/forceLogoutPlatformUserAction/);assert.doesNotMatch(page,/archivePlatformUserAction/);assert.match(page,/ChevronRight/);assert.match(page,/Open \$\{displayName\} user details/);assert.match(page,/SUSPENDED/);assert.match(page,/ARCHIVED/);assert.doesNotMatch(page,/status: { not: "ARCHIVED" }/);assert.match(page,/selectedStatus \? true : user.status !== "ARCHIVED"/);assert.match(detail,/PlatformUserPasswordResetAction/);assert.match(detail,/forceLogoutPlatformUserAction/);assert.match(detail,/archivePlatformUserAction/);assert.match(detail,/UserActionsPanel/);assert.match(detail,/Overview|Account|Business Assignment|Security|Recent Activity|Audit History/);});
